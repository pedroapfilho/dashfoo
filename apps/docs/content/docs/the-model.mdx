...
tabset inside it. See [Dimensions](#dimensions) below.
...
Three keys are tree-wide defaults for the per-node `enable*` flags above:
`tabEnableClose` sits behind both the tabset and the tab `enableClose`,
`tabEnableRename` backs `tab.enableRename`, and `tabSetEnableMaximize` backs
`tabset.enableMaximize`. Set one to `false` to turn the feature off everywhere.
`enableDrag` has no global counterpart; it is per-tab only.
