...
maximized). The model carries only what differs from the defaults, and
`normalize()` fills in the rest of the invariants.
...
**4. Collapse single-child rows.** A row left with exactly one child is
