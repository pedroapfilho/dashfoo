...
mechanism behind it is just two exported pieces. Measure the container with
...
still targets `store.model`, so nothing maps back and widening up is lossless. No
remount: the store, history, and mounted panels survive the breakpoint cross.
