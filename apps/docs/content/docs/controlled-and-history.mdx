...
Second, undo and redo are no-ops in controlled mode. Both early-return
without touching anything or firing `onModelChange`, and `canUndo` /
`canRedo` always report `false`: the controlled sync resets the actor's
history on every update, so its stacks stay empty. There is no hidden
internal stack to fall back on. If you want undo/redo in controlled mode,
you keep the history yourself. That's the
