...
`normalize` enforces the same structural invariants the reducer guarantees: no
empty tabsets, no redundant single-child rows, a live `maximizedTabsetId`. Every
