...
- **Over a tab strip.** The drop stacks into that tabset. A tab lands at a
  specific insertion index, with a tab-sized ghost showing where it will sit; a
...
The on-screen feedback is a tab-sized ghost (`insertionGhostRect`) drawn at the
slot boundary: it takes the dragged tab's width and the vertical box of the
strip's resident tab items, clamped so it never leaves the strip. The slot is
measured against whole tab items (label plus close button), so the "after the
last tab" position sits past the last close button rather than between the
label and the close.
...
- `useTabsetDroppable` registers each tabset's DOM element so the adapter can
  hit-test the pointer against its rect. There are no dnd-kit droppables or
  collision detectors; targeting is dashfoo's own point-in-rect test.
...
its monitor for three events. On `dragstart` it reads the source's data to
decide the subject kind — a grip carries `type: "tabset"`, an external source
carries `type: "external"`, a tab carries its own id — sends `START`, and shows
its own `data-dashfoo="drag-preview"` chip.
On `dragmove` it reads the live pointer, finds the tabset under it, resolves an
intent, and sends `OVER` (positioning the preview chip imperatively, no
re-render). On `dragend` it re-resolves from the final pointer, sends a last
`OVER`, then `DROP`.
...
| ----------------------------- | ---------------------------------------------- | ---------------------- |
| `--dashfoo-dock-fill`         | `oklch(0.556 0 0 / 0.18)`                      | ghost tab + split band |
| `--dashfoo-dock-border`       | `oklch(0.708 0 0 / 0.75)`                      | ghost tab + split band |
| `--dashfoo-dock-border-width` | `1px`                                          | ghost tab + split band |
| `--dashfoo-dock-radius`       | `6px`                                          | ghost tab + split band |
| `--dashfoo-dock-tab-radius`   | `var(--dashfoo-dock-radius, 6px)`              | ghost tab              |
...
- [`tab-insertion.ts`](https://github.com/pedroapfilho/dashfoo/blob/main/packages/react/src/lib/tab-insertion.ts)
  in `@dashfoo/react` for the insertion index, the insertion ghost, and the
  no-op drop filter.
