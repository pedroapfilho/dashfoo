...
a float's rect as you drag or resize it — one undo step per gesture), and
`setFloatMinimized`. `geometry` is in viewport CSS pixels (the overlay is fixed), so
a float can be dragged anywhere on the page. `normalize` heals each float's own
