...
Saved float bounds are projected into the current viewport, so restoring a
desktop layout on a narrow screen keeps title-bar controls reachable. The
projection creates no history entry and never overwrites the saved rect;
widening restores the saved geometry. A later move or resize commits the
geometry visible at that time.
