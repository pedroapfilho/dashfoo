...
and self-heals exactly like a docked panel. The
[anatomy figure](/concepts#anatomy-of-a-layout) shows where floats and their
...
- **Bring to front.** Clicking anywhere in a float — its body, a tab, or its
  chrome — raises it above the others.
- **Honors `editable`.** Under `editable={false}` a float is static: its content
  ...
  minimize, and dock-back are all switched off — none of `moveFloat`, `renameFloat`,
  `setFloatMinimized`, or `dockFloat` can be dispatched from a static layout.
