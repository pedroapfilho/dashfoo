...
description: Float a panel out of the docked layout into a draggable, resizable overlay. Floats are first-class, serializable nodes that stay in the same app.
---
...
- **Same React tree, full drag-dock.** Floats render through the normal `Layout.Rows`
  ...
  tabs into and out of a float** all stay connected.
...
Saved float bounds are projected into the current viewport, so restoring a desktop layout on a narrow screen keeps title-bar controls reachable. This projection does not create history or overwrite the saved size; widening restores the saved geometry. A subsequent move or resize commits the geometry visible at that time.
