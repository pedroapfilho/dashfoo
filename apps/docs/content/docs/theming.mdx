...
The four optional dock tokens from the indicator table above
(`--dashfoo-dock-border-width`, `--dashfoo-dock-radius`,
`--dashfoo-dock-line-radius`, `--dashfoo-dock-transition`) are unset by default
and complete the set: 22 overridable tokens in total.
