...
- An `@theme inline` block bridges the tokens into Tailwind, generating
  ...
  the `[data-dashfoo-theme="dark"]` remap automatically — no `dark:` variant
  needed.
...
| `tabset-popout`      | `<button>`              | Pops the tabset into its own window (shown when `poppable`).              |
...
| `window`             | `<div>`                 | A detached window's frame (in the popup's own document).                  |
| `window-toolbar`     | `<div>`                 | The detached window's top bar with the dock-back control.                 |
| `window-dock`        | `<button>`              | Docks the detached panel back into the main layout.                       |
| `window-body`        | `<div>`                 | The detached window's content area.                                       |
