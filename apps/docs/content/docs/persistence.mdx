...
It is part of the **uncontrolled** half of the props union (you pass `defaultModel`), so passing it with `model` does not compile. In controlled mode,
...
- In the browser, a read that throws (private mode, blocked storage)
  warns and returns `null` rather than crashing.
...
another arrangement. `StorageAdapter` is synchronous; backend synchronization
belongs in controlled mode rather than returning promises from this adapter.
