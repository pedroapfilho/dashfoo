...
payload, debounce-save every change, and flush on page hide and unmount. A `ref` exposes
...
A change still in the debounce window when the page goes away would normally be
lost — a reload or tab close never unmounts React, so the debounce timer simply
dies with the page. Persistence flushes the pending write on `pagehide` and when
the page becomes hidden (`visibilitychange`), as well as in a cleanup effect on
unmount, so reloading right after a drag still saves the last change.
