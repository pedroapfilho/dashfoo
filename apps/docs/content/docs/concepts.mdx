...
description: The names dashfoo uses for tabs, panes, rows, splitters, and floating panels — one annotated figure, the vocabulary, and the same layout as data.
---
...
style, and in the reducer's action names, so it pays to meet them once — in one
picture — before the deeper guides.
...
`TabsetNode`, `TabNode` — and floating panels are a fourth, `FloatNode`, living
in `Dashfoo.floats` beside the tiled tree. The tab strip and the splitters are
...
A note on "window": the floating panel is as close as dashfoo gets. It renders
in the same React tree — never a separate browser window — so context, events,
and styling keep working. See [Floating panels](/floating-panels).

## Row versus column

Rows are the only recursive container. The anatomy figure's right-hand column
is nothing special — just a row with `orientation: "column"` nested inside the
root row:
