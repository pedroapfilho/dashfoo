...
All 18 props. Pass either `model` or `defaultModel`; passing neither throws.
...
| `poppable`                | `boolean`                                                                  | `false` | Show a per-tabset pop-out control that detaches the panel into its own browser window, and render any detached windows (`Dashfoo.windows`). Opt-in. See [detached windows](/detached-windows).                                                                                                                                                                                                                                                                                                                            |
...
| `responsive`              | `{ maxWidth: number; orientation?: Orientation }`                          | —       | At or below `maxWidth` (the layout's own container width, not the viewport) render a stacked column and lock drag + resize. A view-only projection — the model is untouched, so widening up is lossless and the layout never remounts. See [responsive](/responsive).                                                                                                                                                                                                                                                     |
| `snap`                    | `{ step?: number; divisions?: number \| "panels"; threshold?: number }`    | —       | Magnetic snapping for splitter drags. The dragged boundary pulls onto a grid — the union of `step` (multiples of a fixed percent) and `divisions` (even splits: multiples of `100/d`, where `d` is the number, or `"panels"` to divide by the row's panel count so a 3-panel row snaps to thirds). Within `threshold` percent (default `4`) it snaps and commits on release as one undo step. Omitted ⇒ off; replaces (not merges with) `global.snap`; a row can override via its own `snap`. Locked off in compact mode. |
...
| `detachTab`      | `(tabId: string) => void`                                                                       | Pops a tab into a detached window. Call from a user gesture so the browser allows the popup.                   |
| `reattachWindow` | `(windowId: string) => void`                                                                    | Docks a detached window's panel back into the main layout.                                                     |
...
[detached windows](/detached-windows) — `Layout.PopoutLayer` (wrap the tree so
pop-out controls can open popups) and `Layout.Windows` (renders `model.windows`
into detached browser windows; takes `windows` + `global`). Plus the full tabset
decomposition under `Tabset.*`: `Root`, `TabStrip`, `Tablist`, `Tab`, `Trigger`,
`RenameInput`, `CloseButton`, `Content`, `Toolbar`, `OverflowMenu`, `Grip`,
`MaximizeButton`, and `PopoutButton`. Each part spreads native props and is
...
`DashfooLayout`/`Layout.Root` as reactive props — no `key`, no remount. SSR-safe.
...
Every document mutation is one of 16 immutable, discriminated actions. The
...
| `detachTab`              | `{ tabId, geometry?, windowId? }`          | Pop a tab into a new detached window.                     |
| `detachTabset`           | `{ tabsetId, geometry?, windowId? }`       | Pop a whole tabset (with its tabs) into a new window.     |
| `reattachWindow`         | `{ windowId, targetId?, location? }`       | Dock a detached window's panel back into the main layout. |
| `updateWindowGeometry`   | `{ windowId, geometry }`                   | Update a detached window's stored on-screen rect.         |
...
| `windowNode` | `(layout: RowNode, geometry: Geometry, options?: WindowOptions) => WindowNode` | A detached window wrapping a layout subtree at an on-screen rect.              |
...
`maximizedTabsetId`, `windows`), `WindowOptions` (`id`, `name`). `min`/`max`
...
`WindowNode` (`{ type: "window"; id; layout: RowNode; geometry: Geometry; name? }`)
holds a detached window; see [detached windows](/detached-windows). The model's
...
| `normalize` | `(model: Dashfoo) => Dashfoo`                 | The self-healing canonical pass: clamps `selected`, removes empty tabsets, heals each window's layout and drops an emptied window, keeps `maximizedTabsetId` pointing at a live tabset. |

### Tree helpers

Read-only queries over a model. `findRow` and `findTabsetParent` take a
`RowNode` — start them at a root from `collectRoots` (`model.layout` or a
window's layout). Lookups that take the whole `model` span every root, so a
popped-out node is as reachable as a docked one.

| Function             | Signature                                                                             | Description                                                   |
| -------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `collectRoots`       | `(model: Dashfoo) => Array<RowNode>`                                                  | The main layout plus each detached window's layout.           |
...
| `findWindow`         | `(model: Dashfoo, windowId: string) => WindowNode \| undefined`                       | A detached window by id.                                      |
| `findRow`            | `(row: RowNode, id: string) => RowNode \| undefined`                                  | A row by id, within one root.                                 |
| `findRootContaining` | `(model: Dashfoo, nodeId: string) => RowNode \| undefined`                            | Which root (main layout or a window) holds a node.            |
