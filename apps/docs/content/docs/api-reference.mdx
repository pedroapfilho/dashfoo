...
Reaches the layout's resolved configuration and `dispatch` from inside custom
chrome — a `renderTabsetToolbar` button, a custom tab label. Throws when called
outside `<DashfooLayout>`. Returns a `DashfooContextValue`:
...
| `renderTabLabel`, `renderTabsetToolbar`                            | optional render props         | As passed to `DashfooLayout`.                            |
...
The raw context object is also exported as `DashfooContext`.
