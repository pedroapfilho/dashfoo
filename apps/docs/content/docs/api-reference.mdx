...
`tabSetEnableMaximize`) turns it off. The root element is
`<div data-dashfoo="layout">` at 100% of its parent in both axes — a parent
...
| `closableTabs`, `renamableTabs`, `maximizable`, `draggableTabsets` | `boolean`                     | Effective values after the model globals are layered in. |
...
The opt-in default skin: plain CSS over `--dashfoo-*` tokens, light by default,
dark via `data-dashfoo-theme="dark"` on any ancestor. The full token table is in
[theming](/theming).

| Entry point                  | Contents                                                                        |
| ---------------------------- | ------------------------------------------------------------------------------- |
| `@dashfoo/theme/dashfoo.css` | The complete skin. It `@import`s `tokens.css`, so one import styles everything. |
