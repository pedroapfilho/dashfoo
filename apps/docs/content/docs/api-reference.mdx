...
All 18 props. The props are a union: pass **either** `model` (controlled)
...
| `persist`                 | `string \| { key: string; storage?: StorageAdapter; debounceMs?: number }` | none    | Auto-save and restore the model. Only accepted alongside `defaultModel`. A bare string is a localStorage key; the debounce defaults to 300 ms. See [persistence](/persistence).                                                                                                                                                                                                                                                                                                                                          |
...
- `dashfooMachine`: the document actor. Input `{ model: Dashfoo }`, context
  ...
  and `SET_MODEL` (`{ model }`, which resets history). Its context and event
  unions are internal and not exported.
- `dragDockMachine`: the drag/dock interaction lifecycle. Events: `START`
  (`{ subject: DragSubject }`), `OVER` (`{ drop: DropResolution | null }`),
  `DROP`, `CANCEL`. Its context is one `DragState`
  (`{ kind: "idle" } | { kind: "dragging"; subject; drop }`), so a drop target
  without a subject is unrepresentable. A valid drop emits
  `{ type: "COMMIT", action, scope }`, where `scope` echoes back the
  `DropResolution.scope` that resolved the intent so the host can route the
  action: `moveNode` for a tab drag, `moveTabset` for a tabset-grip drag;
  `addNode` only for an external source. `dropAction(drag)` is the same
  decision as a plain function. Types: `DragState`, `DragSubject`,
  `DropResolution`.
