...
| `persist`                 | `string \| { key: string; storage?: StorageAdapter; debounceMs?: number }` | none    | Auto-save and restore the model (uncontrolled only). A bare string is a localStorage key; the debounce defaults to 300 ms. See [persistence](/persistence).                                                                                                                                                                                                                                                                                                                                                              |
...
| `responsive`              | `{ maxWidth: number; orientation?: Orientation }`                          | none    | At or below `maxWidth` (the layout's own container width, not the viewport) render a stacked column and lock drag + resize. A view-only projection: the model is untouched, so widening up is lossless and the layout never remounts. See [responsive](/responsive).                                                                                                                                                                                                                                                     |
...
One exception to watch: `mutableNodeAttrsSchema` is a union of fully optional
objects, so `actionSchema` strips the `attrs` of an `updateNodeAttributes`
payload down to `{}`. Dispatch that action from typed code, or re-attach
`attrs` after parsing. Either way the reducer validates `attrs` against the
target node's own kind and drops keys that belong to another kind.
