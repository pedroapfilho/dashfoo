...
| `floatable`               | `boolean`                                                                  | `false` | Show a per-tabset control that floats the panel into a movable, resizable overlay, and render any floating panels (`Dashfoo.floats`). Opt-in. See [floating panels](/floating-panels).                                                                                                                                                                                                                                                                                                                                    |
...
[floating panels](/floating-panels) — `Layout.FloatLayer` (wrap the tree's content,
...
| `dockFloat`              | `{ floatId, targetId?, location? }`        | Dock a floating panel back into the main layout.         |
...
`FloatNode` (`{ type: "float"; id; layout: RowNode; geometry: Geometry; name? }`)
holds a floating panel; see [floating panels](/floating-panels). The model's
...
| `normalize` | `(model: Dashfoo) => Dashfoo`                 | The self-healing canonical pass: clamps `selected`, removes empty tabsets, heals each float's layout and drops an emptied float, keeps `maximizedTabsetId` pointing at a live tabset. |
...
float's layout). Lookups that take the whole `model` span every root, so a
floated node is as reachable as a docked one.
