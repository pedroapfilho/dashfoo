...
| `normalize` | `(model: Dashfoo) => Dashfoo`                 | The structural canonical pass: removes empty tabsets, collapses single-child rows, heals each float's layout and drops an emptied float, keeps `maximizedTabsetId` pointing at a live tabset. `selected` and `weight` are settled at the parse boundary instead. |
...
- `dashfooMachine`: the document actor. Input `{ model: Dashfoo }`, context
  ...
  `DashfooEvent`, `DashfooMachineInput`.
