...
describe("reducer", () => {
  ...
  test("adjustSplit preserves a collapsed child's expanded weight and records collapsed state", () => {
    ...
    expect(tabsetById(next, "ts1")?.collapsed).toBeUndefined();
    expect(tabsetById(next, "ts1")?.weight).toBe(60);
    expect(tabsetById(next, "ts2")?.weight).toBe(95);
  });

  test("adjustSplit keeps collapse state for collapsible children", () => {
    const input = baseModel();
    ...
    expect(tabsetById(next, "ts1")?.collapsed).toBe(true);
    expect(tabsetById(next, "ts1")?.weight).toBe(60);
    expect(tabsetById(next, "ts2")?.weight).toBe(95);
  });

  test("adjustSplit without collapsedIds keeps existing behavior", () => {
    const input = baseModel();
    const first = input.layout.children[0];
    if (first?.type === "tabset") {
    ...
    const next = reducer(input, { rowId: "root", type: "adjustSplit", weights: [5, 95] });

    expect(tabsetById(next, "ts1")?.collapsed).toBe(true);
    expect(next.layout.children.map((c) => c.weight)).toEqual([5, 95]);
  });

  test("split docking into a collapsed target clears the collapsed state", () => {
    const input = baseModel();
    const first = input.layout.children[0];
    if (first?.type === "tabset") {
      first.collapsed = true;
  ...
  test("maximize while collapsed preserves collapsed state for restore", () => {
    const input = baseModel();
    const first = input.layout.children[0];
    if (first?.type === "tabset") {
      first.collapsed = true;
    ...
    const maximized = reducer(input, { tabsetId: "ts1", type: "setMaximizedTabset" });
    const restored = reducer(maximized, { tabsetId: null, type: "setMaximizedTabset" });

    expect(tabsetById(maximized, "ts1")?.collapsed).toBe(true);
    expect(tabsetById(restored, "ts1")?.collapsed).toBe(true);
  });
